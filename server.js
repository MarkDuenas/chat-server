...
io.on("connection", (socket) => {
  console.log("user connected to ", socket.handshake.query.chatROOM);
  socket.on('disconnect', () => {
    socket.leave(socket.handshake.query.chatROOM)
    console.log('user disconnected from', socket.handshake.query.chatROOM);
  });
  ...
  socket.on("chat message", async (msg) => {
    ...
    await ChatRoom.findByIdAndUpdate({_id: msg.ChatRoomID},{$push:{messages: msg}})
  });
});
